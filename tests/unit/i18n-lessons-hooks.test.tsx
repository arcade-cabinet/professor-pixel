...
describe('useStrings', () => {
  ...
  it('exposes a non-empty catalog (smoke check on shape)', () => {
    const { result } = renderHook(() => useStrings());
    ...
    // strings module owns its shape and changes as copy evolves. But
    // it must not be empty: an empty catalog would be a regression that
    // breaks every consumer silently.
    expect(typeof result.current).toBe('object');
    expect(Object.keys(result.current as object).length).toBeGreaterThan(0);
  });
});
...
describe('useLessons — react-query wrapper', () => {
  ...
  it('uses staleTime=Infinity — remount under the same QueryClient does not refetch', async () => {
    vi.mocked(loadLessons).mockResolvedValue([sampleLesson('a')]);

    // Share ONE QueryClient across both renderHook calls so the second
    // mount actually exercises the cache. (makeWrapper closes over a new
    // client per call, which would defeat the test.)
    const client = new QueryClient({
    ...
    await waitFor(() => expect(result.current.isSuccess).toBe(true));
    expect(loadLessons).toHaveBeenCalledTimes(1);

    unmount();
    const { result: r2 } = renderHook(() => useLessons(), { wrapper });
    // staleTime: Infinity contract — second mount serves cached data
    // synchronously without redriving the loader.
    expect(r2.current.isSuccess).toBe(true);
    expect(r2.current.data).toEqual([sampleLesson('a')]);
    expect(loadLessons).toHaveBeenCalledTimes(1);
  });
});
