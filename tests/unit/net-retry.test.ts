...
describe('defaultShouldRetry — error classification', () => {
  ...
  const cases: Array<{
    ...
    { label: 'message contains "timeout"', error: { message: 'request timeout' }, expectRetry: true },
    ...
    { label: 'Connection failed message', error: { message: 'Connection failed' }, expectRetry: true },
...
describe('fetchWithRetry', () => {
  it('returns response on 2xx', async () => {
    ...
    const promise = retryMechanism.fetchWithRetry('https://example.test/data', {}, { jitter: false });
    await vi.runAllTimersAsync();
...
describe('educationalRetry.withEducationalFeedback', () => {
  it('emits user-friendly messages on each retry', async () => {
    ...
    const promise = educationalRetry.withEducationalFeedback(
      op,
      'loading lesson',
      (m) => messages.push(m)
    );
