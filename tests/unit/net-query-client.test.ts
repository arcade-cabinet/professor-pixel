...
describe('apiRequest', () => {
  ...
  it('handles falsy data correctly (empty string, 0, false do NOT trigger body serialization)', async () => {
    // The implementation uses `data ? ... : undefined` so falsy values
    // (0, '', false, null) skip body serialization. Pin this so a
    // later refactor to allow explicit `null` body doesn't accidentally
    // start sending JSON for falsy primitives.
    stubFetchResolves(makeResponse({ ok: true, status: 200 }));

    await apiRequest('POST', '/api/empty', '');
