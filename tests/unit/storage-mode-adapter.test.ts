...
// Constructor spy lets us count `new ClientStorage()` invocations directly,
// so the singleton-routing test asserts a real invariant (constructor
// called exactly once across multiple StorageAdapter instances) rather
// than a vacuous "two adapters share method spies" check.
const clientStorageCtorSpy = vi.fn();
...
beforeEach(() => {
  vi.resetModules();
  clientStorageCtorSpy.mockReset();
...
describe('getClientStorage — singleton', () => {
  ...
  it('shares the storage across StorageAdapter instances (singleton routing)', async () => {
    ...
    // ClientStorage was constructed exactly ONCE despite two adapters +
    // a getClientStorage() call below — that's the singleton invariant.
    // Without this counter, the previous assertion would also pass if
    // each adapter built its own ClientStorage (since they all share the
    // same fakeStorageMethods object).
    expect(clientStorageCtorSpy).toHaveBeenCalledTimes(1);
