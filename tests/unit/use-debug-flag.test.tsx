...
afterEach(() => {
  // Order: clear listener bookkeeping BEFORE restoring real
...
describe('useDebugFlag — cross-tab live updates', () => {
  it('flips to true when another tab writes localStorage.debug=1', () => {
    ...
    act(() => {
      ...
      // The hook reads only `e.key`, so a plain object cast to StorageEvent
      // suffices — and avoids CodeQL's `js/superfluous-trailing-arguments`
      // false-positive on the StorageEvent constructor (CodeQL's stub
      // doesn't include the WHATWG eventInitDict overload).
      const event = { key: 'debug', newValue: '1' } as unknown as StorageEvent;
  ...
  it('flips to false when another tab clears localStorage.debug', () => {
    ...
    act(() => {
      ...
      const event = { key: 'debug', newValue: null } as unknown as StorageEvent;
      listeners.get('storage')?.forEach((cb) => cb(event));
    });
  ...
  it('also re-reads when a global storage clear is dispatched (key === null)', () => {
    ...
    act(() => {
      ...
      const event = { key: null, newValue: null } as unknown as StorageEvent;
      listeners.get('storage')?.forEach((cb) => cb(event));
    });
  ...
  it('ignores storage events for unrelated keys', () => {
    ...
    act(() => {
      ...
      const event = { key: 'unrelated', newValue: 'x' } as unknown as StorageEvent;
      listeners.get('storage')?.forEach((cb) => cb(event));
    });
