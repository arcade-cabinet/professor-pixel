...
describe('useToast — updating toasts', () => {
  it('handle.update mutates the matching toast in place', () => {
    ...
    act(() => {
      // The hook's `update` signature requires a full ToasterToast,
      // but the reducer's UPDATE_TOAST does { ...t, ...action.toast }
      // and accepts partial updates. `as any` acknowledges the type
      // mismatch — `as never` masks the actual problem.
      // biome-ignore lint/suspicious/noExplicitAny: reducer accepts partial; hook signature is overly strict
      handle?.update({ id: handle.id, title: 'after' } as any);
    });
...
describe('useToast — listener wiring', () => {
  ...
  it('unmount removes the hook from the listeners array', () => {
    // Pin cleanup directly: capture the `result` snapshot AT unmount
    // and observe it does NOT update when subsequent dispatches fire.
    // (If the listener wasn't removed, the unmounted hook's setState
    // would still run and result.current would tick to the new state
    // — which renderHook lets us observe even post-unmount because
    // result is a stable wrapper around the last-rendered value.)
    const { result, unmount } = renderHook(() => useToast());
    ...
    expect(result.current.toasts).toHaveLength(1);
    const toastsAtUnmount = result.current.toasts;

    unmount();

    // A second hook fires a fresh dispatch. If the first hook's
    // listener wasn't removed, its setState would run and
    // result.current.toasts would point to a new array. With proper
    // cleanup, result.current.toasts is referentially identical to
    // the snapshot we took at unmount.
    const { result: r2 } = renderHook(() => useToast());
    act(() => {
      r2.current.toast({ title: 'post-unmount' });
    });

    expect(result.current.toasts).toBe(toastsAtUnmount);
