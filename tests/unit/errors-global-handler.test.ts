...
describe('globalErrorHandler — persistence', () => {
  ...
  it('survives localStorage throwing during persist (private mode)', async () => {
    ...
    // Spy on Storage.prototype.setItem rather than reassigning the
    // instance method directly. In jsdom, instance-level reassignment
    // is unreliable across tests; prototype-level interception is the
    // recommended pattern (see Vitest community best practice).
