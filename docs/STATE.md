...
| Branch | PR | Status |
|--------|----|----|
| `feat/post-30-consolidation` | _local-only_ | Docs/state hygiene PRQ closing everything that fell out of #30. 12 tasks across three blocks: D1–D5 pillar docs (added 06-storage + 07-deploy frontmatter-headed pages; refreshed 02-runtime + 01-frontend with OPFS WASM cache, asset mounting, iOS voiceschanged race, wouter base wrap, edge-swipe, skeleton + aria-busy, AudioToggle chrome; refreshed docs/README.md + AGENTS.md cross-refs); S1–S4 STATE refresh + 12-file plan archive + cursor.md refresh + Next audit; R1–R3 Play Store + iOS TestFlight runbooks + asset-mount cross-link. No code changes. Pushes when queue is fully [x]. |
...
Every line is tagged either `[PRQ:link]` (engineering work has a queue) or `[manual:user-action]` (cannot be automated by the agent — operator must run it).

- **Play Store rollout** `[manual:operator]`. Capacitor Android shell already builds debug APKs on every main push. Operator must: generate the release keystore once (`keytool -genkeypair`), add the four `ANDROID_KEYSTORE_*` repository secrets, run `cd-mobile.yml` with `inputs.release=true`, upload AAB via Play Console. Full runbook: [`DEPLOYMENT.md → Play Store rollout`](./DEPLOYMENT.md#play-store-rollout).
- **iOS TestFlight** `[manual:operator]`. No CI automation — Apple's signing model doesn't fit GitHub-hosted runners cleanly. Mac + Xcode + Apple Developer account required. Full runbook: [`DEPLOYMENT.md → iOS TestFlight`](./DEPLOYMENT.md#ios-testflight).
- **Content tracks** `[manual:design]`. Remaining `**WEAK**` / `**FIX**` items in `docs/playtests/*.md` are flow-JSON content authoring (theme packs, A/B framing of asset pickers, missing scene additions). The dialogue engine supports them today; what's missing is content. Tracked as content-design work, not engineering.
