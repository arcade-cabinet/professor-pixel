...
- **Asset mounting in Pyodide FS.** `compiler.ts` emits `pygame.image.load('/assets/foo.png')` against paths that pygame-ce inside Pyodide cannot resolve — Pyodide's emscripten FS doesn't have those URL-relative paths mounted, so every selected sprite falls through to the try/except's magenta placeholder. The launcher and live preview have been silently degraded since the asset catalog landed; nobody noticed because the placeholder still renders. Fix requires either (a) fetching each selected asset and writing it via `pyodide.FS.writeFile` before runPython, or (b) using pygame-ce's URL-aware `image.load()` if the upstream WASM build supports it. Tracked as separate PR — touches `compiler.ts`, `play.tsx`'s onPlay, and the live-preview path; want a focused review.
