...
describe('getPixelResponse', () => {
  afterEach(() => {
    // Restore Math.random + any other spies even if a prior assertion
    // throws — keeps the Math.random stub from leaking into adjacent
    // tests (dialogFlows describe runs right after this one).
    vi.restoreAllMocks();
  });
