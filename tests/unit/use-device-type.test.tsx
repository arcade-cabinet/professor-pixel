...
function stubViewport(opts: {
  ...
  // msMaxTouchPoints — IE-era third touch signal the hook also OR's
  // in. Pin it deterministically (defaults to 0) so a stale value
  // from another test suite running in the same worker can't bleed
  // through and false-positive isTouchDevice when touch=false.
...
describe('useDeviceType — mobile classification', () => {
  ...
  it('detects touch via msMaxTouchPoints alone (legacy IE/Edge path)', () => {
    // Pin the third OR-branch in the touch detection: ontouchstart and
    // maxTouchPoints both 0/false, but msMaxTouchPoints > 0. Some
    // legacy Edge / Surface devices report touch only via this path.
    delete (globalThis as unknown as Record<string, unknown>).ontouchstart;
    Object.defineProperty(globalThis.navigator, 'maxTouchPoints', {
      configurable: true,
      get: () => 0,
    ...
    vi.stubGlobal('addEventListener', (e: string, cb: () => void) => {
      if (!listeners.has(e)) listeners.set(e, new Set());
      listeners.get(e)!.add(cb);
    });
    vi.stubGlobal('removeEventListener', (e: string, cb: () => void) => {
    ...
    expect(result.current.isTouchDevice).toBe(true);
    // 800 wide + touch + 1100 high → tablet (not foldable: aspect ≈
    // 1.375, hinged-tablet wants width > 820).
